...
const Imageimage =  ({checkToAdd,setCheckToAdd,encryptionFlagCompData}:any) => {
 ...
 /////////////
  ...
  //////////////
  if(firstgroupc4acb?.image){ 
  value = `${firstgroupc4acb?.image}`;
  } else {
  value = `https://placebear.com/g/200/200`;
  }

  const handleCode=async () => {
    let code:any;
    const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:CG:AFGK:TG2:AFK:AllComponents:AFVK:v1",  componentId:"5909e18973e149f19bb5db9d38fc4acb",controlId:"b179abb9c3674612add194ff76654de2",isTable:false,accessProfile:accessProfile,from:"imageImage"},{
      headers: {
  ...
  useEffect(() => {
    setfirstgroupc4acb((pre:any)=>({...pre,image:""}));
    handleCode()
  }, [image54de2?.refresh])

  if (image54de2?.isHidden) {
    return <></>
  } 

  return (
    <div      style={{gridColumn: `1 / 4`,gridRow: `132 / 188`, gap:``, height: `100%`, overflow: 'auto'}}>
        {
          value ? (

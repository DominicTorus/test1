...
const Documentuploaderupload = ({checkToAdd,setCheckToAdd,refetch,setRefetch}:any) => {
    ...
    try{     
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "7b1ddc3a9535415e83b78afebaee78d7",
          isTable: false,
          from:"ButtonUpload",
  ...
  const handleClick = async (file:any) => {
    setfirstgroupc4acb((prev: any) => ({ ...prev, upload: file }))
      handleCustomCode()
    }

  if (uploade78d7?.isHidden) {
    return <></>
  }

  return (
    <div 
      style={{gridColumn: `4 / 6`,gridRow: `7 / 18`, gap:``, height: `100%`, overflow: 'auto'}} >
      <TorusDocumentUploader
        className=""
        id="uploade78d7"
        value={firstgroupc4acb.upload}
        dropzoneOptions={{

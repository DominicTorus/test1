  ...
  const handleMapperValue = async()=>{
    const orchestrationData: any = await AxiosService.post(
      '/UF/Orchestration',
      {
        ...
        controlId: "9dc0590133b545eda17e3e67518c9a9f",
        isTable: false,
  ...
  const getDropdownData = async(value?:any)=>{
    let te_refreshBody:te_refreshDto={
        ...
        if(te_refreshData?.data?.error == true){
          ...
          //setdfd_state_dropdown_v1Props(te_refreshData?.data?.dataset?.data || [])
        }
    let dfData = te_refreshData?.data?.dataset?.data
    let mapperColumn: string =  `statevalue`
    let mapperText: string =  `statecode`
...
if(!dropdownData.COUNTRY && Array.isArray(dropdownData.COUNTRY)){
  ...
  useEffect(() => {
      ...
      if (!dropdownData.COUNTRY) {
        ...
        delete temp.code
        delete temp.statevalue
        setthirdgroup01a8f(temp)
      }
    getDropdownData()
  },[statec9a9f?.refresh,dropdownData.COUNTRY])

  const handlechange = async(value: any) => {
    if(value.length>0){
      setthirdgroup01a8f((prev: any) => ({ ...prev, code: getMapperDetailsBindValues[value[0]],statevalue: value[0] }))
  ...
  useEffect(() => {
    if(Array.isArray(dfd_state_dropdown_v1Props) && dfd_state_dropdown_v1Props?.length == 1){
    // setthirdgroup01a8f((pre:any)=>({...pre,code:dfd_state_dropdown_v1Props[0]?.code}))
    }
  },[dfd_state_dropdown_v1Props])

  const selected=useRef({})
  const handleClick=async(value?:any)=>{
    if (value.length > 0) {
      setthirdgroup01a8f((prev: any) => ({ ...prev, code: getMapperDetailsBindValues[value[0]]}))
         setIsRequredData(false)
    } else {
       setthirdgroup01a8f((prev: any) => ({ ...prev, code: ''}))
        setIsRequredData(true)
    }
    setError('')
    setValidate((pre:any)=>({...pre,code:undefined}))
  ...
  useEffect(() => {
    if(initialCount!=0)
     setthirdgroup01a8f((pre:any)=>({...pre,code:""}))
    else
      setInitialCount(1)
  },[statec9a9f?.refresh])

  if (statec9a9f?.isHidden) {
    return <></>
  }

  return (
    <div 
      style={{gridColumn: `6 / 8`,gridRow: `63 / 78`, gap:``, height: `100%`, overflow: 'auto'}} >
      <div>
      </div>
      <Select   
        className=""    
        disabled= {statec9a9f?.isDisabled ? true : false}
        width = {250}
        placeholder={keyset("State")} filterable={true} hasClear={true} onUpdate={handlechange} value={thirdgroup01a8f?.statevalue ? [thirdgroup01a8f?.statevalue] : []}
        >
